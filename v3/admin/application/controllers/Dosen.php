...
class Dosen extends CI_Controller
{
	...
	public function getDataDosen()
	{
		...
		$data = [
			"prodi" => isset($_GET['prodi']) ? $_GET['prodi'] : 0,
			"status" => @$_GET['status'],
			"fakultas" => isset($_GET['fakultas']) ? $_GET['fakultas'] : $session_data['idFakultas']
		];
	...
	{
		$res = $this->DosenModel->getDataDosenAll();
	...
	public function keluarHomeBaseDosenIndividu()
	{
		$data['idDosen'] = $_POST['idDosenKeluar'];
		$res = $this->DosenModel->keluarHomeBaseDosenIndividu($data);
		if ($res) {
			echo json_encode("Data Dosen Berhasil Keluar Dari Homebase");
		} else {
			echo json_encode("Delete Gagal");
		}
	}
