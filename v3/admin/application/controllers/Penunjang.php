...
/*-------------------------------------------------- View Controller ------------------------------------*/
	...
	public function submitDataSK()
	{
		...
		$no_sk        = $_POST['sk'];
		$tgl_sk       = $_POST['tgl_sk'];
		$nama_keg     = str_replace(array("'",'"'), ' ', $_POST['nama_keg']);
			...
			if($keg_bln == "1"){
				for($x=0;$x<$jml_keg;$x++){
					...
					$deskripsi = array(
						...
						'dari'           => $jml_keg,
						'keg_perbln'     => 1,
						'uuid_penunjang' => $dUuid[0]['uuid']
					);
					$data['deskripsi'] = json_encode($deskripsi);
					$data['tgl_keg'] = $pTglKeg[0].'-'.($pTglKeg[1]+$x).'-'.$pTglKeg[2];
	...
	public function getDataSKBulanTahun()
	{
		...
		$datakirim        = array();
		$datakegiatan     = $this->KegiatanModel->getSKPenunjangBulanTahun($data);
		$deskripsi        = array();
	...
	{
		$dd                    = explode("#", $_GET['no_sk']);
		$data['no_sk']         = $dd[0];
		$data['tanggal_entry'] = $dd[1];
		$session_data          = $this->session->userdata('sess_admin');
		$data['id_user']       = $session_data['name'];
		$datakegiatan          = $this->KegiatanModel->getSKNoSKbyDate($data);
		$deskripsi             = array();
		$datakirim             = array();
		foreach($datakegiatan as $row)
		{
	...
	{
		$dd                       =explode("#",$_POST['id_kegiatan_dosen']);
		$data['no_sk']            = $dd[0];
		$data['tanggal_kegiatan'] = $dd[1];
		$data['tanggal_entry']    = $dd[2];
		$session_data             = $this->session->userdata('sess_admin');
		$data['id_user']          = $session_data['name'];
		print_r($data);
		...
		{			
			...
			foreach($databaru as $row)
			{
				...
				$deskripsi = array(
					...
					'uuid_penunjang' => $deskripsilama['uuid_penunjang'],
   				);
				$data['deskripsi'] = json_encode($deskripsi);
				$data['nip']       = $row['id_dosen'];
