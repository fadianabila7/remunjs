...
class GolonganModel extends CI_Model
{
	...
	{
		$sql = "SELECT rg.*, g.* from riwayat_golongan rg JOIN golongan g ON rg.id_golongan = g.id_golongan WHERE rg.id_dosen= '" . $data['id_dosen'] . "' ORDER BY rg.tmt desc";

		$res = $this->db->query($sql);
		return $res->result_array();
	}
	public function getGolonganKode($data)
	{
		$sql = "SELECT * FROM golongan WHERE golongan='" . $data['golongan'] . "'";
		$res = $this->db->query($sql);
		return $res->result_array();
	}
	...
	{
		$sql = "INSERT INTO riwayat_golongan VALUES ('','" . $data['id_dosen'] . "','" . $data['golongan'] . "','" . $data['tmt'] . "')";
		print_r($sql);
	...
	public function getIdDosen($data)
	{
		$sql = "SELECT id_dosen from dosen where nip='" . $data['idDosen'] . "'";
		$res = $this->db->query($sql);
		return $res->result_array();
	}
