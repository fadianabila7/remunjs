...
class TambahanModel extends CI_Model {
	...
	public function getSKTambahanBulanTahun($data)
	{
		if($data['bulan']==0)
		{
			...
			$sql ="SELECT DISTINCT(kd.no_sk_kontrak) as no_kontrak,k.induk,k.satuan,
					(select deskripsi from kegiatan_dosen kd2 where kd2.no_sk_kontrak=kd.no_sk_kontrak limit 0,1) as desk,
					(select tanggal_kegiatan from kegiatan_dosen kd2 where kd2.no_sk_kontrak=kd.no_sk_kontrak limit 0,1) as tgl_keg
					FROM kegiatan_dosen kd JOIN kegiatan k ON kd.kode_kegiatan = k.kode_kegiatan 
					JOIN admin adm ON kd.id_user = adm.id_user
					where YEAR(kd.tanggal_kegiatan)='".$data['tahun']."' AND k.kode_jenis_kegiatan = '5'
		...
		else
		{
			...
			$sql = "SELECT DISTINCT(kd.no_sk_kontrak) as no_kontrak,k.induk,k.satuan,
					...
					k.kode_jenis_kegiatan = '5' and adm.id_fakultas='".$data['fakultas']."'";
		}
	...
	public function deleteKegiatanDosenBySK($data)
	{
		...
		$sql = "DELETE FROM kegiatan_dosen WHERE deskripsi like '%uuid_tambahan\":\"".$data['uuid']."\"%'";
		$res = $this->db->query($sql);
		return $res;
	}
