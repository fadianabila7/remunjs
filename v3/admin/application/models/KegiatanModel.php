...
class KegiatanModel extends CI_Model {
	...
	public function getKegiatanIndividuJenisBulanTahun($data)
	{
		...
		$sql = "SELECT kd.*, k.bobot_sks,k.keg_bulanan  FROM kegiatan_dosen kd JOIN kegiatan k ON kd.kode_kegiatan = k.kode_kegiatan WHERE k.kode_jenis_kegiatan = '".$data['jenis_kegiatan']."' AND kd.id_dosen='".$data['nip']."' AND kd.tanggal_kegiatan < date('".$tempYear."-".$tempMonth."-01') AND kd.tanggal_kegiatan >= date('".$currentYear."-".$currentMonth."-01')";
	...
	public function getSKPenunjangBulanTahun($data)
	{
		if($data['bulan']==0)
		{
			...
			$sql ="SELECT DISTINCT kegiatan_dosen.`no_sk_kontrak` as no_kontrak, kegiatan_dosen.`deskripsi` as desk, 
					...
					WHERE admin.`id_fakultas` = ".$data['fakultas']."
					AND YEAR(kegiatan_dosen.`tanggal_kegiatan`)= ".$data['tahun']."
					AND MONTH(kegiatan_dosen.tanggal_kegiatan)=".$data['bulan']."
					AND kegiatan.`kode_jenis_kegiatan`=4
					group by kegiatan_dosen.no_sk_kontrak, kegiatan_dosen.tanggal_entry";
		}
		$res = $this->db->query($sql);
	...
	{
		$sql = "SELECT * FROM kegiatan_dosen kd where no_sk_kontrak = '".$data['no_sk']."' and id_user='".$data['id_user']."' ORDER BY kode_kegiatan";
		$res = $this->db->query($sql);
		return $res->result_array();
	}

	public function getSKNoSKbyDate($data){
		$sql = "SELECT * FROM kegiatan_dosen kd where no_sk_kontrak = '".$data['no_sk']."' and tanggal_entry = '".$data['tanggal_entry']."' and id_user='".$data['id_user']."' GROUP BY id_dosen ORDER BY deskripsi ASC";
		$res = $this->db->query($sql);
		return $res->result_array();
	}
	...
	public function deleteKegiatanDosenBySK($data)
	{
		...
		$sql = "DELETE FROM kegiatan_dosen WHERE no_sk_kontrak = '".$data['no_sk']."' and tanggal_entry = '".$data['tanggal_entry']."' and id_user = '".$data['id_user']."'";
		$res = $this->db->query($sql);
		return $res;
	}
