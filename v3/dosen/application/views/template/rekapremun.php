        ...
        <div id="page-content">
            <div class="row wrapper border-bottom white-bg dashboard-header">
                <div class="col-lg-10">
                    <h2>Rekapitulasi Pembayaran Remunerasi</h2>
                    <ol class="breadcrumb">
                        <li>
                            <a href="<?php echo site_url(); ?>">Home</a>
                        </li>
                        <li class="active">
                            <strong>Rekap Remunerasi</strong>
                        </li>
                    </ol>
                </div>
                <div class="col-lg-2">
                    ...
                    <div class="col-lg-12">
                        <div class="ibox float-e-margins">
                            <div class="ibox-title">
                                <h5>Rekapitulasi Remunerasi Bulanan</h5>
                                                ...
                                                <div class="col-md-3">
                                                    <select class="form-control m-b" name="tahun">
                                                        ...
                                                        for ($i = 2019; $i <= date('Y'); $i++) {
                                                            echo "<option " . ($i == date('Y') ? 'selected' : '') . ">" . $i . "</option>";
                                                        }
                                                        ?>
                                                    </select>
                                                </div>
                                        ...
                                        <table id='datatable' class="table table-striped table-bordered table-hover dataTables-example">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <!-- <th>Bendahara</th> -->
                                                    <th>Bulan</th>
                                                    <th width="10%">Σ SKS Gaji dibayar (%)</th>
                                                    <th width="10%">Σ SKS Kinerja dibayar (%)</th>
                                                    <th>Tarif Gaji(Rp.)</th>
                                            ...
                                            <tbody id="table-body" align="right">
                                            </tbody>
                                        </table>
                                    </div>
                                    <div>
            ...
            <script>
                ...
                function convertStatus(status) {
                    switch (status) {
                        case -1:
                            return "Kegiatan Tidak Valid";
                            break;
                        case -2:
                            return "Kegiatan Valid";
                            break;
                        case -3:
                        ...
                        case -4:
                            ...
                            break;
                        case 0:
                            return "Belum Diproses Bendahara Fakultas";
                            break;
                        case 1:
                            return "Sudah Diproses Bendahara Fakultas";
                            break;
                        case 2:
                            return "Sudah Dibayarkan UNRI";
                            break;
                    }
                }
                ...
                function getRekapIndividu() {
                    ...
                    $.ajax({
                        url: "<?php echo base_url(); ?>index.php/MainControler/getDataRekapPembayaran",
                        type: "GET",
                        data: postData,
                        success: function(ajaxData) {
                            ...
                            for (var i = 0; i < result.length; i++) {
                                texBulan = convertMonth(result[i]['bulan']);
                                textStatus = convertStatus(result[i]['status']);
                                bersih = (Math.floor(result[i]['total']) - Math.floor(result[i]['pajak']));
                                sumbayar = (eval(sumbayar) + eval(bersih));
                                t.row.add([
                                    i + 1,
                                    // result[i]['bendahara'],
                                    texBulan,
                                    result[i]['sks_gaji'] * 100,
                                    result[i]['sks_kinerja'] * 100,
                                    rupiah(result[i]['tarif_gaji']),
                                    rupiah(result[i]['tarif_kinerja']),
                                    rupiah(Math.floor(result[i]['total'])),
                                    rupiah(Math.floor(result[i]['pajak'])),
                                    "<b>" + rupiah(bersih) + "</b>",
                                    textStatus,
                                ]).draw();
                            }
                            $("#sumbayar").empty();
                            $("#sumbayar").append("Rp. " + rupiah(sumbayar) + ",00");
                        },
                        error: function(status) {
                            t.clear().draw();
                        }
