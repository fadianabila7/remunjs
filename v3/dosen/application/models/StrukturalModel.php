...
class StrukturalModel extends CI_Model {
	...
	{
		$sql = "SELECT rs.*, js.* FROM riwayat_jabatan_struktural rs LEFT JOIN jabatan_struktural js ON rs.id_jabatan_struktural = js.id_jabatan_struktural WHERE rs.id_dosen= '".$data['nip']."' ORDER BY rs.tmt desc, rs.id_riwayat_jabatan_struktural DESC";
	...
	{
		$sql = "SELECT r.id_riwayat_jabatan_struktural, r.deskripsi, j.* FROM riwayat_jabatan_struktural r JOIN jabatan_struktural j ON r.id_jabatan_struktural = j.id_jabatan_struktural WHERE r.tmt < date('".$data['tahun']."-".$data['bulan']."-10') AND r.id_dosen = '".$data['nip']."' order by r.tmt DESC, r.id_riwayat_jabatan_struktural DESC LIMIT 1 ";
		$res = $this->db->query($sql);
