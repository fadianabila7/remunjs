...
class FungsionalModel extends CI_Model {
	...
	{
		$sql = "SELECT r.id_riwayat_jabatan_fungsional, r.id_jabatan_fungsional , f.* FROM riwayat_jabatan_fungsional r JOIN jabatan_fungsional f ON r.id_jabatan_fungsional = f.id_jabatan_fungsional WHERE r.id_dosen='".$data['nip']."' AND r.tmt < date('".$data['tahun']."-".$data['bulan']."-10') order by r.tmt DESC LIMIT 1";
