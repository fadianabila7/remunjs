...
class DosenModel extends CI_Model {
	...
	public function getDosen($data)
	{
		if($data['prodi'] == 0){
			...
			if($data['status']==0){
				$sql = "SELECT d.*,p.nama as namaprodi, s.deskripsi FROM dosen d JOIN program_studi p ON d.id_program_studi = p.id_program_studi JOIN status_dosen s ON d.id_status_dosen = s.id_status_dosen where d.id_program_studi in (".$data['prodi'].")";
			}else{
				$sql = "SELECT d.*,p.nama as namaprodi, s.deskripsi FROM dosen d JOIN program_studi p ON d.id_program_studi = p.id_program_studi JOIN status_dosen s ON d.id_status_dosen = s.id_status_dosen where d.id_program_studi in (".$data['prodi'].") AND d.id_status_dosen='".$data['status']."'";
			}
		}
	...
	public function DosenJurusan($data){
		$sql = "SELECT d.*,p.nama as namaprodi, s.deskripsi 
				FROM dosen d JOIN program_studi p ON d.id_program_studi = p.id_program_studi 
				JOIN status_dosen s ON d.id_status_dosen = s.id_status_dosen 
				where p.id_jurusan in (".$data['jurusan'].") AND d.id_status_dosen='".$data['status']."'";
	...
	{
		$sql = "SELECT d.*,p.nama as namaprodi, f.nama as namafakultas, f.singkatan, s.deskripsi FROM dosen d JOIN program_studi p ON d.id_program_studi = p.id_program_studi JOIN fakultas f ON p.id_fakultas = f.id_fakultas JOIN status_dosen s ON d.id_status_dosen = s.id_status_dosen WHERE d.id_dosen = '".$data['nip']."'";
		$res = $this->db->query($sql);
		return $res->result_array();
	}
	...
	public function getDosenJurusan($session_data){
		$sql="SELECT program_studi.id_jurusan from dosen join program_studi on program_studi.id_program_studi = dosen.id_program_studi where id_dosen='".$session_data['user']."'";
		$res = $this->db->query($sql);
		return $res->result_array();
	}
}