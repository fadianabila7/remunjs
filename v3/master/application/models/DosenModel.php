...
class DosenModel extends CI_Model
{
	...
	public function getDosen($data)
	{
		...
		$this->db->join('program_studi p', 'd.id_program_studi = p.id_program_studi');
		$this->db->join('status_dosen s', 'd.id_status_dosen = s.id_status_dosen');
		...
		// 	else
		// 	{
		// 		$sql = "SELECT d.*,p.nama as namaprodi, s.deskripsi FROM dosen d JOIN program_studi p ON d.id_program_studi = p.id_program_studi JOIN status_dosen s ON d.id_status_dosen = s.id_status_dosen where p.id_fakultas = '".$data['fakultas']."' AND ";
		// 	}
		// }
		// else
		// {
