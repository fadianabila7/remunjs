...
class OperatorModel extends CI_Model {
	...
	{
		$sql = "SELECT o.*, p.nama as namaprodi, f.nama as namafakultas FROM operator o JOIN program_studi p ON o.id_program_studi = p.id_program_studi JOIN fakultas f ON p.id_fakultas = f.id_fakultas JOIN user u ON o.id_user = u.id_user WHERE o.id_operator='".$data['operator']."'";
		$res = $this->db->query($sql);
		return $res->result_array();
	}
