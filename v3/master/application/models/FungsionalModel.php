...
class FungsionalModel extends CI_Model {
	...
	public function getFungsionalByTMT($data){
		$sql = "SELECT r.id_riwayat_jabatan_fungsional, r.id_jabatan_fungsional , f.grade, f.jobvalue, f.nama as namaf, f.sksr_maks_gaji, f.sksr_maks_kinerja FROM riwayat_jabatan_fungsional r JOIN jabatan_fungsional f ON r.id_jabatan_fungsional = f.id_jabatan_fungsional WHERE r.id_dosen='".$data['nip']."' AND r.tmt < date('".$data['tahun']."-".$data['bulan']."-05') order by r.tmt DESC LIMIT 1";
		$res = $this->db->query($sql);
		return $res->result_array();
	}

	public function getFungsionalByRange($data,$bulan){
		$sql = "SELECT (f.sksr_maks_gaji+f.sksr_maks_kinerja) as sksr_max FROM riwayat_jabatan_fungsional r JOIN jabatan_fungsional f ON r.id_jabatan_fungsional = f.id_jabatan_fungsional WHERE r.id_dosen='".$data['nip']."' AND r.tmt < date('".$data['tahun']."-".$bulan."-05') order by r.tmt DESC LIMIT 1";
		$res = $this->db->query($sql);
		return $res->result_array();
	}

	public function getFungsionalByTMTWR($data)
	{
		$sql = "SELECT r.id_riwayat_jabatan_fungsional, r.id_jabatan_fungsional , f.* FROM riwayat_jabatan_fungsional r JOIN jabatan_fungsional f ON r.id_jabatan_fungsional = f.id_jabatan_fungsional WHERE r.id_dosen='".$data['nip']."' AND r.tmt < date('".$data['tahun']."-".$data['bulan']."-05') order by r.tmt DESC LIMIT 1";
