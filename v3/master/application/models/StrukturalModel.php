...
class StrukturalModel extends CI_Model {
	...
	public function getStrukturalByRange($data,$bulan)
	{
		$sql = "SELECT r.id_riwayat_jabatan_struktural, r.deskripsi, j.* FROM riwayat_jabatan_struktural r JOIN jabatan_struktural j ON r.id_jabatan_struktural = j.id_jabatan_struktural WHERE r.tmt < date('".$data['tahun']."-".$bulan."-05') AND r.id_dosen = '".$data['nip']."' order by r.tmt DESC, r.id_riwayat_jabatan_struktural DESC LIMIT 1 ";
		$res = $this->db->query($sql);
