...
class Pengajaran extends CI_Controller {
	...
	public function do_insertMembimbingTa(){
		...
		if(!$id_dosen=="" && !$deskripsi==""){
			for($x=0;$x<$jml_bulan;$x++){
				...
				if($ptanggal[2]>=25){ $ptanggal[2]='25'; }
	...
	public function tambahkelaskuliah(){
		...
		$data =  array(	'dataKegiatan' => $dataKegiatan , 
	...
	public function do_editmatakuliah($id_matkul){
		...
		if(!empty($kode_matkul) && !empty($namaMatkul) && !empty($sks) && !empty($tahunkurikulum) && !empty($status)){
			$data_edit = array(
					...
					'kode' => $kode_matkul,
					'nama' => $namaMatkul,
					'sks' => $sks,'tahun_kurikulum' => $tahunkurikulum,
					'status_aktif' => $status,);
			try{
	...
	public function detail_bimbingan(){
		$input          = $this->input->post();
		$ex             = explode('-',$input['data']);
		$filter['like'] = array('deskripsi' => '"uuid_bimbing":"'.$ex[1].'"');
		$model          = $this->Model->get_kegiatan_dosen($filter);
		$data           = array("result" => "success", "file" => $model);
