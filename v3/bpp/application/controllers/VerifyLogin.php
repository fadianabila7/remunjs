...
class VerifyLogin extends CI_Controller
{
		...
		if ($result && $result[0]->id_role == 4) {
			...
			foreach ($result as $row) {
				...
				$sess_array = array(
					...
					'idBendahara' => $row->id_bendahara,
					'namafakultas' => "Fakultas " . $row->namaf,
				);
				$this->session->set_userdata('sess_bpp', $sess_array);
			}
	...
	{
		$CI = &get_instance();
		$log_path = '../../logs/bpp/login/log_' . date('Y-m-d') . '.log';
