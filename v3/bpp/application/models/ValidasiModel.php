...
class ValidasiModel extends CI_Model {
	...
	{
		$sql = "INSERT INTO validasi_tridharma VALUES ('','".$data['nip']."', '".$data['bulan']."', '".$data['tahun']."','".$data['sks_bid_1']."','".$data['sks_bid_2']."', '".$data['sks_bid_3']."', '".$data['sks_bid_4']."', '".$data['sks_bid_5']."','".$data['sksr_tugas_tambahan']."', '".$data['sksr_gaji']."', '".$data['sksr_kinerja']."', '".$data['sksr_sisa']."', '".$data['deskripsi']."', '".$data['status']."')";
	...
	{
		$sql = "UPDATE validasi_tridharma SET jumlah_sks_bid_1 = '".$data['sks_bid_1']."', jumlah_sks_bid_2 = '".$data['sks_bid_2']."', jumlah_sks_bid_3 = '".$data['sks_bid_3']."', jumlah_sks_bid_4 = '".$data['sks_bid_4']."', jumlah_sks_bid_5 = '".$data['sks_bid_5']."',  sksr_tugas_tambahan = '".$data['sksr_tugas_tambahan']."', sksr_gaji = '".$data['sksr_gaji']."', sksr_kinerja = '".$data['sksr_kinerja']."', sksr_sisa = '".$data['sksr_sisa']."', deskripsi = '".$data['deskripsi']."', status = '".$data['status']."' WHERE id_dosen = '".$data['nip']."' AND bulan = '".$data['bulan']."' AND tahun='".$data['tahun']."'";
		$res = $this->db->query($sql);
		return $res;
	}
